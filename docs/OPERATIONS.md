...
메모리 Fab 통합 화면은 `/` 또는 `/index.html`, 모델 근거 검증은 `/evidence.html`, CMOS Fab 시뮬레이터는 `/cmos-lab.html`, 기존 포토 실험실은 `/photo-lab.html`에서 열 수 있습니다. 해당 실험은 브라우저 로컬 기록으로 별도 보관됩니다. 워크스페이스의 버전을 ‘3D 장비에서 확인’으로 열면 해당 조건을 불러옵니다. 변경한 조건은 ‘현재 조건으로 변경안 작성’을 눌러 별도 버전으로 저장합니다. 기존 브라우저 로컬 기록은 자동으로 서버에 업로드하지 않습니다.

### 업데이트 후 새 화면이 404인 경우

화면 파일을 갱신해도 실행 중인 Python 서버 코드는 자동으로 바뀌지 않습니다. `0.4.0` 이하에서 업데이트할 때는 기존 서버를 실행한 터미널에서 `Ctrl+C`로 종료한 다음 **같은 포트**로 다시 실행합니다. 예를 들어 기존 주소가 `127.0.0.1:8770`이면 다음을 사용합니다.
...
위 두 명령 중 하나만 실행합니다. 포트를 바꾸면 브라우저 로컬 기록의 저장 영역도 바뀝니다. `/api/status`의 `version`으로 실행 중인 서버 버전을 확인할 수 있습니다.

`0.4.1`부터 공개 파일 목록은 `server/public-assets.json`에서 요청마다 읽습니다. 새 프런트엔드 파일과 목록을 함께 배포하면 서버를 재시작하지 않아도 새 화면에 접근할 수 있습니다. Python 코드나 의존성을 변경한 경우에는 여전히 재시작해야 합니다. 목록에는 공개할 화면·스크립트·스타일만 넣으며 계측 데이터, DB, 소스 백업은 넣지 않습니다. 목록이 손상되거나 없으면 파일 제공을 중단하고 503 오류를 반환합니다.
